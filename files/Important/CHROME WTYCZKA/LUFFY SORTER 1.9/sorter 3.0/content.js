...
// Opóźnione ładowanie całej logiki o 200 ms
setTimeout(() => {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.type === 'serviceOrderUpdated') {
            ...
            // Obsługa wszystkich ustawień naraz
    ...
    // Pobierz wszystkie ustawienia z localStorage
    ...
    // Funkcje porównania dla każdego kryterium
    ...
    chrome.storage.onChanged.addListener(function(changes, namespace) {
        if (namespace === 'sync' && changes.visibleRows) {
            ...
            if (changes.serviceOrder) {
                currentServiceOrder = changes.serviceOrder.newValue;
            }
            if (changes.servicePriorities) {
                servicePriorities = changes.servicePriorities.newValue;
            }
            if (changes.sortingHierarchy) {
                sortingHierarchy = changes.sortingHierarchy.newValue;
            }
            if (changes.enableFiltering) {
                enableFiltering = changes.enableFiltering.newValue;
            }
            if (changes.minPriorityToShow) {
                minPriorityToShow = changes.minPriorityToShow.newValue;
            }
            sortAndFilterTable();
        }
    });
}, 200); // Opóźnienie 200 ms